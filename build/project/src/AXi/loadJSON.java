...
public class loadJSON {
	...
	public static AreaInfo printMap (JSONObject map)throws IOException, JSONException{
		...
		for (int i=0; i<objectives.length(); i++){
			...
			String ii = new String("./Icons/" + node.getString("owner") + "/" + nn.getIcon(node.getInt("id")));
			//System.out.println("Node " + nodeName + " belongs to " + node.get("owner"));
			nodes[i] = new GWNode(ii, nodeName, node.getString("owner"));
		}
		...
		for (int i=0; i<bonuses.length(); i++){
			...
			nodes[18] = new GWNode(null, node.getString("type"), node.getString("owner"));
		}
		//System.out.println();
		ai.nodes = nodes;
		return ai;
	...
	protected static ImageIcon createImageIcon(String path) {
		 java.net.URL imgURL = loadJSON.class.getResource(path);
		 if (imgURL != null) {
